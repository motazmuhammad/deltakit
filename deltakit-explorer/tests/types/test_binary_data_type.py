...
@pytest.mark.parametrize(
    "cls",
    [BinaryDataType, Measurements, LeakageFlags, DetectionEvents, ObservableFlips],
)
@pytest.mark.parametrize(
    "target_format", [DataFormat.F01, DataFormat.B8, DataFormat.CSV]
)
...
@pytest.mark.parametrize(
    "cls",
    [BinaryDataType, ObservableFlips, DetectionEvents, Measurements, LeakageFlags],
)
@pytest.mark.parametrize("data_format", [DataFormat.B8, DataFormat.CSV, DataFormat.F01])
@pytest.mark.parametrize("times", [1, 2, 3000])
def test_eq_ok(cls, data_format, times):
    assert cls([[0, 1] * times], data_format) == cls(
        DataString("01" * times), DataFormat.F01
    )
...
@pytest.mark.parametrize(
    ("cls1", "cls2"),
    [(BinaryDataType, ObservableFlips), (DetectionEvents, Measurements)],
)
