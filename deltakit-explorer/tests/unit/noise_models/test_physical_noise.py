...
class TestPhysicalNoise:
    def test_physical_noise(self, noise_params):
        ...
        assert set(
            noise_parameters.measurement_flip.keys()
        ) == gates.MEASUREMENT_GATES - {gates.HERALD_LEAKAGE_EVENT}
        ...
        assert not noise_parameters.gate_noise[0](
            noise_context
        )  # I gates are not included in `gate_noise`
        ...
        assert (
            not noise_parameters.measurement_noise_before
        )  # no additional noise before measurements

        noise_context = NoiseContext(circuit, circuit.layers[5])
        noise_channel = noise_parameters.measurement_noise_after[0](noise_context)[0]
        assert isinstance(noise_channel, noise_channels.Depolarise1)
        assert noise_channel.probability == noise_params["p_meas_qubit_error"]
        assert not noise_parameters.reset_noise[0](
            noise_context
        )  # MRZ is treated as measurement, not reset

        # Check idle noise against _idle_noise_from_t1_t2
        t = 1e-6
        noise_context = NoiseContext(circuit, circuit.layers[0])
        noise_channel_result = noise_parameters.idle_noise(noise_context, t)
        noise_channel_reference = _idle_noise_from_t1_t2(
            noise_params["t1"], noise_params["t2"]
        )(noise_context, t)
        assert isinstance(noise_channel_result, noise_channels.PauliChannel1)
        assert (
            noise_channel_result.probabilities == noise_channel_reference.probabilities
        )
